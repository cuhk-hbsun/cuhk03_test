...
def train(model,
    ...
    with h5py.File(h5_path,'r') as f:
        ...
        for i in xrange(train_num):
            ...
            model.fit_generator(Data_Generator.flow(f,flag = flag_train),one_epoch,epoch_num,validation_data=Data_Generator.flow(f,train_or_validation=which_val_data,flag=flag_val),validation_steps=nb_val_samples, workers=1)
            prnit('finish model.fit_generator')
            Rank1s.append(round(cmc(model)[0],2))
            print Rank1s
            # print ('Rank1s : %s' %(Rank1s))
